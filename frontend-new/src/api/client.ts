...
const API_BASE = "http://192.168.21.137:8000";
...
export async function saveSearch(userId: number, query: string, sport?: string, minPrice?: number, maxPrice?: number) {
  const { data } = await api.post("/saved-searches", { user_id: userId, query, sport, min_price: minPrice, max_price: maxPrice });
  return data;
}
